...
$(document).ready( function () {
	...
	columns.push({'data':'branch', 'type': 'string', 'title': 'Branch/Bank ID'});
	columns.push({'data':'branchName', 'type': 'string', 'title': 'Branch/Bank Name'});
	columns.push({'data':'contraId', 'type': 'string', 'title': 'Contra ID'});
	...
	columns.push({'data': 'conta_Id', 'type': 'image', 'access': 'Edit','title':'Edit', 'url': './ContraEdit.html?action=edit&id='});
	columns.push({'data': 'conta_Id', 'type': 'image', 'access': 'Delete','title':'Delete', 'url': './ContraDelete.html?searchOption=true&action=delete&id='});
	$(this).mainGridTable(columns, 'ContraViewDO', 'conta_Id');
									...
									<div class="card" style="">
										...
										<div class="card-block">
											<div class="table-responsive">
											         <input type="search" id="txt_searchall" class="form-control col-sm-2 float-right" placeholder="Enter Contra ID to Search">
													...
													<a href="ContraCreate.html" class="btn btn-success float-right btn-sm" role="button"><i class="feather icon-file-minus"></i>Create</a>
                                                     <table id="key-act-button" class="table table-bordered table-striped table-condensed" style="width: 100%">
	                                                	<thead class="thead-dark">
	                                                   </thead>
	                                                   </table> 
                                            </div>
											     <table class="searchTable" id="searchId" style="display:none;"><tr><td><input type="text" id="contraId"  class="textStyle_180 tableField dt_input"/></td> </tr> </table>
											</div>
										</div>
									</div>
								</div>
